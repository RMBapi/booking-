...
export const BookingsAndContacts: React.FC<BookingsAndContactsProps> = ({
  ...
  const { bookings, isLoading: bookingsLoading, refetch: refetchBookings } = useGetBookings(
    businessId,
    undefined
  );
  const { contacts, isLoading: contactsLoading } = useGetContacts(businessId);
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter((booking) => {
      ...
      if (!matchesSelectedDate(bookingStart)) {
        return false;
      }
      ...
      const notes = booking.customerNotes?.toLowerCase() || "";
  ...
  const filteredContacts = useMemo(() => {
    return contacts.filter((contact) => {
      ...
      if (!contactStart) return false;

      if (!matchesSelectedDate(contactStart)) {
        return false;
      }
      ...
      const serviceName = contact.service?.name?.toLowerCase() || "";
          ...
          <thead className="border-b border-stone-100 bg-stone-50/60">
            <tr>
              ...
              <th className="px-6 py-6 text-left text-xs font-semibold uppercase tracking-[0.22em] text-stone-400">
                {activeTab === "bookings" ? "Customer" : "Contact"}
              </th>
              <th className="px-6 py-6 text-left text-xs font-semibold uppercase tracking-[0.22em] text-stone-400">
                Service
              </th>
