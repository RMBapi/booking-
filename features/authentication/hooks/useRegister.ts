  ...
  >({
    ...
    onSuccess: async (response, variables) => {
      ...
      if (accessToken && (!user || Object.keys(user).length === 0)) {
        console.warn(
          "⚠️ User object is empty/missing in registration response. Fetching user profile...",
        );
        ...
        try {
          ...
          console.error(
            "❌ Error fetching user profile after registration:",
            profileError,
          );
          handleError(
            new Error(
      ...
      if (!user || !accessToken) {
        ...
        handleError(
          new Error(
            "Invalid response: missing user or token. Check backend /auth/register endpoint",
          ),
        );
        return;
      }
      ...
      if (userRoles.length === 0) {
        // Backend may omit roles on register; fall back to the requested role.
        ...
        if (isDev) {
          console.warn(
            "User object missing roles; using registration role as fallback:",
            registrationRole,
          );
        }
      }
      ...
      handleSuccess(
        `Registration successful as ${registrationRole.replace("_", " ")}!`,
      );
